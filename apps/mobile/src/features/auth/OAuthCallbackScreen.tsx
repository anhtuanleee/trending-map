...
export function OAuthCallbackScreen({ returnTo }: Props) {
  ...
  if (user) return <Redirect href={safeReturnTo as never} />;
    ...
    >
      ...
      <Text style={styles.description}>
        {failed
          ...
          : 'Mạch Phố đang xác minh session Google và đưa mày về thao tác trước đó.'}
      </Text>
      {failed ? (
        <Pressable
          style={styles.button}
          onPress={() => {
            resetOAuthError();
            router.replace({ pathname: '/auth', params: { returnTo: safeReturnTo } });
          }}
        >
...
const styles = StyleSheet.create({
  ...
  errorIconWrap: { backgroundColor: '#fcebea' },
